...
def voc_fewshot(base_dir, split, transforms, to_tensor, labels, n_ways, n_shots, max_iters, n_queries=1):
    ...
    for label in labels:
        with open(os.path.join(voc._id_dir, voc.split,'class{}.txt'.format(label)), 'r') as f:
            # read all element in the class file, like  class1.txt. (class1.txt - class20.txt)
            # f.read().splitlines() without '\n' compare to f.readlines().
    ...
    # numpy.bincount() function. https://blog.csdn.net/xlinsist/article/details/51346523
    ...
    # Create paired dataset. create # max_iters of <S,Q>.
