...
# the same part code notes go to train.py 
@ex.automain
def main(_run, _config, _log):
    ...
    # notrain = True for test stage just using Pretrained VGG16 initialization to Segment
    # and without training on VOC / COCO dataset. this condition stand for “PANet-init" model.
    ...
    # test stage don't need calculate gradient for each tensor. this can save gpu memory.
    with torch.no_grad():
        # test model in test dataset #n_runs round.
        for run in range(_config['n_runs']):
            _log.info(f'### Run {run + 1} ###')
            # let each round test dataset is different.
            ...
            for sample_batched in tqdm.tqdm(testloader):
                ...
                # using bounding box as mask for weak annotations.
                if _config['bbox']:
                    support_fg_mask = []
                    support_bg_mask = []
                    for i, way in enumerate(sample_batched['support_mask']):
                        ...
                        for j, shot in enumerate(way):
                            fg_mask, bg_mask = get_bbox(shot['fg_mask'], sample_batched['support_inst'][i][j])
            ...
            # get the metric for this round.
            classIoU, meanIoU = metric.get_mIoU(labels=sorted(labels), n_run=run)
            classIoU_binary, meanIoU_binary = metric.get_mIoU_binary(n_run=run)

            # write the metrics to log file.
            ...
            # output the metrics to terminal.
    ...
    # get the metric for this test stage.
