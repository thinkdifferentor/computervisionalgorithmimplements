...
class Metric(object):
    ...
    def record(self, pred, target, labels=None, n_run=None):
        ...
        # should deep understand.
        for j, label in enumerate(labels):
            # Get the location (index) of the pixels that are predicted as class j. 
            # For predict class j and exclude background. pred has got argmax at call the func.
            idx = np.where(np.logical_and(pred == j, target != 255))
            pred_idx_j = set(zip(idx[0].tolist(), idx[1].tolist()))
            # Get the location (index) of the pixels that are class j in ground truth.
    ...
    def get_mIoU(self, labels=None, n_run=None):
        ...
        if n_run is None:
            # get the sum of TP\FP\FN at each round.
